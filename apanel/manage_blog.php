<?php
	...
	$category_res=$db->getData("blog_category","*","isdelete=0");

	/*$module_id = 0;
	if( isset($_REQUEST['module_id']) && $_REQUEST['module_id'] > 0 )
		$module_id = $_REQUEST['module_id'];

	$module_name = $db->getValue('module', 'name', 'id='. $module_id);*/
?>
...
<body id="page-top">
													...
													<div class="row">
														...
														<div class="col-md-6 ">
															<div class="form-group category-select">
																<select class="form-control" name="category" id="category" onchange="function_category()">
																	<option value="">Search By Category</option>
																	<?php
																		while($category_row=mysqli_fetch_assoc($category_res))
																		{
																			echo "<option value='".$category_row['id']."'>".$category_row['title']."</option>";
																		}
																	?>
																</select>
															</div>
	...
	function displayRecords(numRecords) {
		...
		var data_url    = "<?php echo ADMINURL; ?>ajax_get_<?php echo $ctable; ?>.php?show=" + numRecords + "&searchName=" + searchName + "&category=" + category + "";
		$("#results").html("");
		$(".loading-div").show();
