<?php
	...
	if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
		$ctable_where .= " (
					blog_title like '%".$_REQUEST['searchName']."%' 
		) AND ";
	}
			...
			<tbody>
				...
				if(@mysqli_num_rows($ctable_r)>0){
					...
					<tr>
						...
						<td><?php echo $ctable_d['blog_title']; ?></td>
						<?php
							$ctable_res=$db->getData("blog_category","*","id=".$ctable_d['category_id']." AND isdelete=0");
							$ctable_row=mysqli_fetch_assoc($ctable_res);

						?>
						<td><?php echo $ctable_row['title']; ?></td>
						<td>
